...
(ref:cap-fcc-hist) **Historical forest cover change for each study-area**. Forest cover areas are given in thousands of hectares (Kha) for the years 2000, 2010 and 2020 ("fc2000", "fc2010", and "fc2020", respectively). The mean annual deforested area $d$ for the 2010--2020 period is given in hectare per year (ha/yr). For comparison of the deforestation intensity between study-areas, the corresponding mean annual deforestation rate $p$ on the period 2010--2020 is also provided in percent per year (%/yr), with one decimal precision.\vspace{0.5cm} 
...
## Arrange data
df <- df %>% 
  dplyr::select(area_name, area_code, for2000, for2010, for2020, andef, pdef) %>%
  mutate_at(.var=vars(starts_with("for")),.fun=function(x){round(x/1000)})
## Make table
vect.align <- c(rep("l", 2), rep("r", 5))
col.names <- c("Country -- Study-area", "Code", "fc2000", "fc2010", "fc2020", "$d$ (ha/yr)", "$p$ (\\%/yr)")

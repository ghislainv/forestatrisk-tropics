...
(ref:fcc-hist) **Historical forest cover change for each study-area**. blabla.\vspace{0.5cm} 
...
## Arrange data
df <- df %>% 
  select(code, for2000, for2010, for2020, andef, pdef) %>%
  mutate_at(.var=c(2,3,4),.fun=function(x){round(x/1000)})
## Make table
vect.align <- c(rep("l", 1), rep("r", 5))
col.names <- c("Code","for2000","for2010","for2020","d (ha)","p (%)")
knitr::kable(df.cat, caption="(ref:cap-samp-size)", booktabs=TRUE, longtable=TRUE,
						 format.args=list(big.mark=","),
						 escape=FALSE,
						 col.names=col.names,

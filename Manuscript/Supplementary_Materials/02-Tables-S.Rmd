...
(ref:cap-c-em) **Cumulative carbon emissions associated to future deforestation**. We computed the cumulative carbon emissions associated to future deforestation from 2020 for each study-area. To do so, we used our maps of projected forest cover change together with the aboveground biomass map by @Avitabile2016. The aboveground biomass map was not covering some islands in the tropics (e.g. Reunion island or Fidji for example). \vspace{0.5cm} 
...
df <- df %>%
  ...
  dplyr::na_if(0) %>%
  mutate_at(.var=vars(starts_with("C")),.fun=function(x){round(x/1000000)}) %>%

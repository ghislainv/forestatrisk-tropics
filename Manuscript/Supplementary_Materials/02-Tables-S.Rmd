...
## Import data
df <- read.table("tables/fcc_by_region.csv", header=TRUE, sep=",")
## Arrange data
df <- df %>% 
...
knitr::kable(df, caption="(ref:cap-fcc-hist-reg)", booktabs=TRUE, longtable=TRUE,
						 format.args=list(big.mark=","),
						 escape=FALSE,
						 col.names=col.names,
						 align=vect.align, linesep="") %>%
...
df <- df %>%
  dplyr::mutate(loss21=round(100*(for2000-for2100)/for2000)) %>%
  # Here, be careful to take year 2000 as a reference
  mutate(yr95dis=floor(2000 + (for2000*0.95)/andef)) %>%
  dplyr::mutate(yr95dis=as.character(yr95dis)) %>%
  dplyr::select(area_cont, for2040, for2060, for2080, for2100, loss21, yr95dis) %>%
  mutate_at(.var=vars(starts_with("for")),.fun=function(x){round(x/1000)})
## Make table
vect.align <- c(rep("l", 1), rep("r", 6))
col.names <- c("Country -- Study-area", "fc2040", "fc2060", "fc2080", "fc2100", "loss21 (\\%)", "yr95dis")
knitr::kable(df, caption="(ref:cap-fcc-proj-reg)", booktabs=TRUE, longtable=TRUE,
						 format.args=list(big.mark=","),
						 escape=FALSE,
						 col.names=col.names,
						 align=vect.align, linesep="") %>%
